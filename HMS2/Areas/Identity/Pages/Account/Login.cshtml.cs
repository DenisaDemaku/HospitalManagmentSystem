...
namespace HMS2.Areas.Identity.Pages.Account
{
    [AllowAnonymous]
    public class LoginModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var user = await dbContext.Users.FirstOrDefaultAsync(x => x.Email.ToLower() == Input.Email.ToLower());
                if (result.Succeeded&& user !=null)
                {
                   var userRoles=await _userManager.GetRolesAsync(user);
                    if (userRoles.Contains("Patient"))
                    {
                        return RedirectToAction("Index", "Appointments", new { area = "Pacient" });
